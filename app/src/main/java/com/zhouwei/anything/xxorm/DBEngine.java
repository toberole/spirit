...
public class DBEngine {
    ...
    public synchronized void init(Context context) {
        if (!isInit) {
            ...
            DBConfigHelper helper = new DBConfigHelper(context);
            int currentVersion = helper.get();
            try {
                dbConfig = DBConfig.getInstance();
                int b = dbConfig.parserDBConfig(context.getAssets().open("db.xml"), currentVersion);
                if (currentVersion != b) {
                    helper.insert(b);
